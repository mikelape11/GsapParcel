...
const VISITOR = {
  ...
  CallExpression(path, asset) {
    ...
    if (isRequire) {
      ...
      if (!parent.isProgram() || path.findParent(p => p.isConditionalExpression() || p.isLogicalExpression() || p.isFunction())) {
        ...
        if ((0, t().isImportNamespaceSpecifier)(specifier)) {
          ...
          for (let p of [...(0, _nullthrows().default)(path.scope.getBinding(specifier.local.name)).referencePaths]) {
            ...
            if ((0, t().isIdentifier)(node) && (0, t().isMemberExpression)(parent, {
              object: node
            }) && ((0, t().isIdentifier)(parent.property) || parent.computed && (0, t().isStringLiteral)(parent.property))) {
              var _parent$property$name, _dep$symbols$get;

              let imported = // $FlowFixMe
              (_parent$property$name = parent.property.name) !== null && _parent$property$name !== void 0 ? _parent$property$name : parent.property.value;
